	...
	for(int i=fila;i<fila+3;i++){
		for(int j=columna;j<columna+3;j++){
			...
			if(Matriz[i][j].Valor == NumeroBuscado && Matriz[i][j].BloqueDisp[NumeroBuscado-1] != 0){
				//cout<<"Encontré el numero: "<<NumeroBuscado<<" en la casilla: "<<i<<","<<j<<" bloque "<<SubMatriz<<endl;
				for(int k=0;k<9;k++){					
...
int RevisionBloque(int SubMatriz,int NumeroBuscado,Celda Matriz[][9]){
	...
	if(cont == 8){
		...
		//Matriz[iaux][jaux].BloqueDisp[NumeroBuscado-1] = 0;	
		for(int r=0;r<9;r++){
...
void Revisar(Celda Matriz[][9], int NumeroBuscado){
	bool var = true;
	while(var){
	int b=0,f=0,c=0,lb=0,n=0,b2=0,f2=0,c2=0,lb2=0,cont=0;
